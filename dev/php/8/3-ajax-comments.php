...
switch ($_POST['req']) {
  ...
  case "show";
    // (B1) GET ALL COMMENTS
    try {
      $stmt = $pdo->prepare("SELECT `name`, `timestamp`, `message` FROM `comm` WHERE `post_id`=? ORDER BY `timestamp` ASC");
  ...
  case "add":
    ...
    // (C2) INSERT
    try {
      $stmt = $pdo->prepare("INSERT INTO `comm` (`post_id`, `name`, `message`) VALUES (?,?,?)");
